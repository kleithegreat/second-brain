...
for col_name in rulerData.columns:
    if col_name not in ["frame_no", "timestamp", "size_px-hotpink", "position_px_x-hotpink", "position_px_y-hotpink", "size_px-darkorange", "position_px_x-darkorange", "position_px_y-darkorange"]:
        del rulerData[col_name]

for col_name in puckData.columns:
    if col_name not in ["frame_no", "timestamp", "position_px_x-green", "position_px_y-green"]:
        del puckData[col_name]

puckData.dropna(subset=["position_px_y-green"], inplace=True) # drops all empty rows



### RULER STUFF
def distance(row):
    return sqrt( (row["position_px_x-hotpink"] - row["position_px_x-darkorange"])**2 + (row["position_px_y-hotpink"] - row["position_px_y-darkorange"])**2 )
...
# conversion factor
metersPerPixel = 0.6 / averageLength
print(f"Meters per pixel: {metersPerPixel}")
# error propagation
mPPError = (0.6 / averageLength) * sqrt((0.0005/0.6)**2 + (avgLenSEM/averageLength)**2)
...
period = 3
puckData["xposavg"] = puckData["position_px_x-green"].rolling(window=period).mean()
puckData["yposavg"] = puckData["position_px_y-green"].rolling(window=period).mean()
...
plt.plot(puckData["time_s"], puckData["position_px_y-green"], label="Raw y position")
plt.plot(puckData["time_s"], puckData["xposavg"], label="Smoothed x position")
plt.plot(puckData["time_s"], puckData["position_px_x-green"], label="Raw x position")
...
plt.plot(puckData["time_s"], puckData["xaccelavg"], label="Smoothed x acceleration")
plt.plot(puckData["time_s"], puckData["xaccel"], label="Raw x acceleration")
