...
puck_colors = ("darkorange", "yellowneon")

# Sorted from smallest to largest angle
file_names = ("data\\2.csv", "data\\1.csv", "data\\3.csv", "data\\4.csv",
              "data\\7.csv", "data\\obtuse2.csv", "data\\8.csv", "data\\5.csv")

# Start and end timestamps for each angle before collision
timestamps_before = [(2785.6, 2917.7), (1947.2, 2079.2), (2842.9, 3042.6), (2446.9, 2546), 
                     (2016.6, 2316.6),(3477.3, 3942),(38578.4, 38942.7), (2713, 2911.6)]

# Start and end timestamps for each angle after collision
timestamps_after = [(3117.6, 3385.7), (2279.3, 2379.2), (3275, 3507.2), (2746.3, 3349.4), 
                    (2522.7, 2748.7), (4277.9, 4677.9), (39310.5, 39510.7), (3285.2, 3779.5)]
...
def contact_timestamp(df: pd.DataFrame) -> any:
    ...
    if dist.size == 0:
        raise ValueError("The distance array is empty")
...
def clean_data(df: pd.DataFrame, time_range: int) -> pd.DataFrame:
    ...
    # Removing extra rows
    mask = (df["timestamp"] >= contact_timestamp(df) - time_range) & (
                df["timestamp"] <= contact_timestamp(df) + time_range)
...
def isolate(df: pd.DataFrame, t1: float, t2: float) -> pd.DataFrame:
    """
...
def main():
    ...
    for i, v in enumerate(file_names):
        df = isolate(pd.read_csv(v), timestamps_before[i][0], timestamps_after[i][1])
        key = f"{float(find_angle(isolate(df, timestamps_before[i][0], timestamps_before[i][1]), contact_timestamp(df), degree=True)):.2f}"
        data[key] = df

    #export_csv(data, 1000)

    # Momentum x before and after collision
    fig, ax = plt.subplots()

    x = data.keys()
    p0 = [find_momentum(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]), True)[0]
          for i, v in enumerate(x)]
    pf = [find_momentum(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]), True)[0]
          for i, v in enumerate(x)]
    error_p0 = [find_momentum(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]), True)[1]
                for i, v in enumerate(x)]
    error_pf = [find_momentum(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]), True)[1]
                for i, v in enumerate(x)]
    ...
    x = data.keys()
    p0 = [find_momentum(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]), False)[0] for i, v in
          enumerate(x)]
    pf = [find_momentum(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]), False)[0] for i, v in
          enumerate(x)]
    error_p0 = [find_momentum(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]), False)[1] for i, v in
                enumerate(x)]
    error_pf = [find_momentum(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]), False)[1] for i, v in
                enumerate(x)]
    ...
    x = data.keys()
    ratio = [find_momentum(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]), True)[0] /
             find_momentum(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]), True)[0] for i, v in
             enumerate(x)]
    ...
    x = data.keys()
    ratio = [find_momentum(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]), False)[0] /
             find_momentum(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]), False)[0] for i, v in
             enumerate(x)]
    error = [ratio[i] * np.sqrt((find_momentum(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]), True)[1] / find_momentum(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]), True)[0])**2 + (find_momentum(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]), True)[1] / find_momentum(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]), True)[0])**2) for i, v in enumerate(x)]
    ...
    x = data.keys()
    ke0 = [find_kinetic_energy(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]))[0]
           for i, v in enumerate(x)]
    kef = [find_kinetic_energy(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]))[0]
           for i, v in enumerate(x)]
    error_ke0 = [find_kinetic_energy(isolate(data[v], timestamps_before[i][0], timestamps_before[i][1]))[1]
                 for i, v in enumerate(x)]
    error_kef = [find_kinetic_energy(isolate(data[v], timestamps_after[i][0], timestamps_after[i][1]))[1]
                 for i, v in enumerate(x)]
