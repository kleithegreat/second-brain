...
class ModelCar(gym.Env):
    ...
    def do_action(self, act):
        ...
        #batt_change = battery_level_start - np.mean(self.battery_level)
        #new_min = (1 - batt_change/2) * motor_min
        #new_stop = (1 - batt_change) * motor_stop
        # batt = np.mean(self.battery_level)
        # if batt > 197:
        #     self.motor_offset += 500
        # elif batt < 195:
        #     self.motor_offset -= 500

        real_throttle = motor_stop-self.motor_offset
        print('throttle', (1/real_throttle)*1000000)
        motor_val = compute_pwm_value(act[1], -1, 1260000, motor_stop, motor_stop+80000)
        if act[1] == 0.0: motor_val = 0.0
        send_action(self.conn, (act[0], float(motor_val)))
    ...
    def reset(self, **kwargs):
        if self.state is None:
            ...
            if self.use_localization:
                ...
                self.do_action([0., 0.])
        for i in range(20):
            self.state, _ = self.do_action([0., 0.])
            time.sleep(1)
        self.initial_battery_level = np.mean(self.battery_level)
        diff = battery_level_start - self.initial_battery_level
        print('batt diff', diff, 'suggested', diff*5000)
        self.motor_offset += (diff*1500)
        time.sleep(3)
